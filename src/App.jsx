...
const PRIOR_MEAN = 80;         // Bayesiaans gemiddelde
const PRIOR_WEIGHT = 10;       // virtuele vragen
const MIN_ANS_FOR_BEST = 5;    // threshold
...
const GlobalStyle = () => (
  <style>{`
    ...
    @keyframes pofPop { 0% { transform: scale(0.6); opacity: 0; } 20% { transform: scale(1.12); opacity: 1; } 50% { transform: scale(1.0); } 100% { transform: scale(0.9); opacity: 0; } }
...
/* ---------- standaard vragen ---------- */
const DEFAULT_VRAGEN = [/* ... jouw bestaande lijst ... */];
...
// === Stap 2: Anonymous Auth (onzichtbaar, geen UI) ===
const auth = getAuth(firebaseApp);
signInAnonymously(auth).catch(() => { /* ignore */ });

/* ---------- seed + local storage helpers ---------- */
...
/* ---------- persistente speler-naam (alleen naam in localStorage) ---------- */
...
/* ====== NIEUW: leave policy + presence helpers ====== */
...
export default function PimPamPofWeb() {
  ...
  // Gebruik auth.uid als speler-id
  const playerId = uid;
  ...
  // Auth guard: render pas na uid
  if (!playerId) {
    return (
      <>
        <GlobalStyle />
        <div style={{ padding: 16 }}>Verbinden…</div>
      </>
    );
  }
  ...
  function makeRoomCode(len = 5) { let s = ""; for (let i = 0; i < len; i++) s += CODE_CHARS[Math.floor(Math.random() * (i + 1)) % CODE_CHARS.length]; return s; }
  ...
  // ===== Undo / Pauze / Submit / Change Letter / Jilla =====
              ...
              <table className="table">
                ...
                <tbody>
                  {matches.map(m => {
                    ...
                    const result = placement === 1 ? "Gewonnen" : (placement ? ordinalLocal(placement) : "—");
    ...
    <>
      ...
      <div style={styles.wrap}>
        <header style={styles.header}>
          ...
          {(offlineSolo || (roomCode && room?.started)) && (
        ...
        {(!roomCode || (roomCode && isHost && !room?.started)) && !offlineSolo && (
          <>
