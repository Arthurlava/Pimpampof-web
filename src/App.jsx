...
export default function PimPamPofWeb() {
    ...
    const [vragen, setVragen] = useState(() => {
        try {
            ...
            if (!raw) { const seeded = seed(); localStorage.setItem(STORAGE_KEY, JSON.stringify(seeded)); return seeded; }
            let parsed; try { parsed = JSON.parse(raw); } catch { const seeded = seed(); localStorage.setItem(STORAGE_KEY, JSON.stringify(seeded)); return seeded; }
            if (!Array.isArray(parsed) || parsed.length === 0) { const seeded = seed(); localStorage.setItem(STORAGE_KEY, JSON.stringify(seeded)); return seeded; }
    ...
    async function leaveRoom() {
        ...
        await runTransaction(r, (data) => {
            ...
            const ids = data.players ? Object.keys(data.players) : [];
            if (!ids.length) { actuallyLeft = true; return null; }
    ...
    function onLetterChanged(e) {
        ...
        if (val.length === 1) {
            ...
            if (isOnlineRoom && isMyTurn && myJailCount === 0 && !inCooldown) {
                const required = normalizeLetter(room?.lastLetter);
                if (required && required !== "?" && val === required) triggerPof(`Dubble pof! +${DOUBLE_POF_BONUS}`);
