...
export function WhatsNewPanel({ whatsNew, isOpen, onToggle }) {
  ...
  const visibleUpdates = showOlderUpdates ? updates : updates.slice(0, latestCount);
  const hasOlderUpdates = updates.length > latestCount;

  return (
    <Section title="Recente updates">
      <Row>
        <span className="muted" style={{ maxWidth: 520, textAlign: "left" }}>
          De laatste wijzigingen in de game.
        </span>
        ...
        <>
          <div style={{ marginTop: 12, display: "flex", flexDirection: "column", gap: 14 }}>
            {visibleUpdates.map((update) => (
              <div key={update.updatedAtLabel}>
                <h3 style={{ margin: "0 0 6px 0", fontSize: 15, textAlign: "left" }}>
                  {update.updatedAtLabel}
                </h3>
                <ul style={{ margin: "0 0 0 18px", textAlign: "left", lineHeight: 1.55 }}>
                  {update.items.map((text, index) => (
                    <li key={`${update.updatedAtLabel}-${index}`} style={{ marginBottom: 6 }}>{text}</li>
                  ))}
                </ul>
              </div>
            ))}
          ...
          {hasOlderUpdates && (
            <div style={{ marginTop: 12 }}>
              <Button variant="alt" onClick={() => setShowOlderUpdates((show) => !show)}>
